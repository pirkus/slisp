...
use cli::{compile_file_to_executable, DebugFeature};
...
fn main() {
    ...
    let mut debug_features: HashSet<DebugFeature> = HashSet::new();
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--dump-ir" => {
                debug_features.insert(DebugFeature::DumpIr);
                i += 1;
            }
            "-o" => {
                if i + 1 >= args.len() {
    ...
    if trace_alloc {
        debug_features.insert(DebugFeature::TraceAllocations);
    }

    if compile_mode {
        if let Some(input) = input_file.as_deref() {
            ...
            match compile_file_to_executable(input, output, keep_object, &debug_features) {
                Ok(()) => {
                    if !debug_features.contains(&DebugFeature::DumpIr) {
                        println!("Successfully compiled file '{}' to '{}'", input, output);
                    }
                }
        ...
        println!("  slisp --compile [--keep-obj] [--dump-ir] [-o <out>] <file>  - Compile .slisp file to executable");
