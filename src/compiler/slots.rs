...
impl SlotTracker {
    ...
    /// Check if any slots are being tracked.
    ...
    pub fn apply_liveness_and_release(
        self,
    ...
    /// Release all temporary slots without applying liveness (for cases where freeing is manual).
    pub fn release_all(self, context: &mut CompileContext) {
        self.temp_slots.into_iter().for_each(|slot| {
            context.release_temp_slot(slot);
        });
    }

    /// Get the list of owned slots (for manual processing).
...
mod tests {
    ...
    fn track_if_owned_respects_ownership() {
        ...
        let none_result = tracker.track_if_owned(
            &mut instructions,
            &mut context,
            HeapOwnership::None,
            ValueKind::String,
        ...
        let owned_result = tracker.track_if_owned(
            &mut instructions,
            &mut context,
            HeapOwnership::Owned,
            ValueKind::String,
