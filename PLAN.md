...
- ✅ **6.4.1 Vector runtime primitives:** Designed a heap-backed vector layout with runtime helpers for create/access/clone/free and surfaced interpreter support for `vec`, `count`, `get`, and `subs`-style slicing, mirroring string semantics with borrowed inputs and cloned returns. Telemetry workloads (`churn_reuse`, `escaping_strings`, `mixed_sizes`) ran cleanly via `tests/programs/memory/run_allocator_telemetry.sh` with zero outstanding allocations.
  ...
  - ✅ **6.4.13 Multi-pass compilation for type inference (COMPLETED WITH LIMITATIONS):** Fixed the core issue where `ValueKind::Any` was incorrectly assumed to be `String` in function return cloning logic (functions.rs:65-95). The fix distinguishes between parameters (which can be returned borrowed without cloning) and local variables (which must be cloned before returning). Removed the incorrect `Any` → `String` assumption that was causing spurious clones. **Status:** With multi-pass DISABLED, all unit tests pass (163/163) and integration tests show 43/47 passing (same as baseline). Multi-pass remains DISABLED (mod.rs:151) due to semantic differences in type-optimized IR generation. **Root cause identified:** When pass 3 uses learned type information, it generates fundamentally different IR than pass 2 (e.g., `RuntimeCall("_string_normalize", 2)` vs `RuntimeCall("_string_from_number", 1)` for string parameters). This causes runtime behavior differences leading to test failures. **Implementation details:** Created infrastructure for multi-pass compilation with fresh context per pass and proper type information propagation. Preserved the multi-pass framework in `compile_program_multipass` for future work. **Future work:** Ensure type-optimized IR in pass 3 produces semantically equivalent runtime behavior to pass 2, possibly through: (1) more conservative type-based optimizations, (2) runtime type guards, or (3) unified normalization functions that handle both typed and untyped values correctly.
  - **6.4.14 Spillover ergonomics:** Introduce destructuring helpers or higher-order utilities that lean on the new containers, and wire telemetry harnesses to capture allocator pressure under mixed workloads.
