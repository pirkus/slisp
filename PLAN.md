...
- ✅ **6.4.1 Vector runtime primitives:** Designed a heap-backed vector layout with runtime helpers for create/access/clone/free and surfaced interpreter support for `vec`, `count`, `get`, and `subs`-style slicing, mirroring string semantics with borrowed inputs and cloned returns. Telemetry workloads (`churn_reuse`, `escaping_strings`, `mixed_sizes`) ran cleanly via `tests/programs/memory/run_allocator_telemetry.sh` with zero outstanding allocations.
  ...
  - ⚠️ **6.4.13 Multi-pass compilation for type inference (PARTIAL):** Implemented three-pass compilation strategy to properly infer parameter types from call sites. **Pass 1:** Register all function signatures. **Pass 2:** Compile into temporary program gathering type information from call sites via `record_function_parameter_type` with propagation back to parent context. **Pass 3:** Recompile with known parameter types so `str()` and other operations use correct runtime functions based on actual types instead of defaults. Added string literal preservation between passes to fix index misalignment (src/compiler/mod.rs:200). **Status:** Successfully fixes original bug (debug_bug1.slisp - returns 5 instead of 7) but introduces 6 test regressions due to extra `_string_clone` instructions being generated. Root cause identified: functions.rs:71-74 assumes `Any` types are strings and adds clones, interacting badly with type inference. **Currently enabled** at src/compiler/mod.rs:150. Current: 38/47 passing (43/47 without multi-pass). Multi-pass IS working for its intended purpose; remaining issues are with clone generation strategy. See BUG_FIX_SUMMARY.md and MULTIPASS_BUG_INVESTIGATION.md for details.
