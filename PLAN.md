...
- ✅ **6.4.1 Vector runtime primitives:** Designed a heap-backed vector layout with runtime helpers for create/access/clone/free and surfaced interpreter support for `vec`, `count`, `get`, and `subs`-style slicing, mirroring string semantics with borrowed inputs and cloned returns. Telemetry workloads (`churn_reuse`, `escaping_strings`, `mixed_sizes`) ran cleanly via `tests/programs/memory/run_allocator_telemetry.sh` with zero outstanding allocations.
  ...
  - **6.4.3 Map runtime groundwork:** Finalise hashing/equality semantics, design the heap layout, surface interpreter APIs (`assoc`, `dissoc`, `get`, `contains?`), and back them with targeted regression programs plus allocator telemetry baselines.
  - **6.4.4 Set runtime groundwork:** Build on the map primitives to supply membership-focused helpers (`set`, `contains?`, `disj`), confirm borrowed/owned behaviour, and add interpreter fixtures that stress uniqueness and churn before compiler work begins.
  - **6.4.5 Spillover ergonomics:** Introduce destructuring helpers or higher-order utilities that lean on the new containers, and wire telemetry harnesses to capture allocator pressure under mixed workloads.
