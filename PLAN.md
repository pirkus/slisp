...
- ✅ **6.4.1 Vector runtime primitives:** Designed a heap-backed vector layout with runtime helpers for create/access/clone/free and surfaced interpreter support for `vec`, `count`, `get`, and `subs`-style slicing, mirroring string semantics with borrowed inputs and cloned returns. Telemetry workloads (`churn_reuse`, `escaping_strings`, `mixed_sizes`) ran cleanly via `tests/programs/memory/run_allocator_telemetry.sh` with zero outstanding allocations.
  ...
  - ✅ **6.4.12 Get operation jump patching:** Fixed critical bug where jump instructions in `get` operations (for vectors, maps, and strings) were being patched BEFORE liveness analysis ran. When liveness inserted `FreeLocal` instructions to clean up heap-allocated values, it shifted instruction indices, causing jump targets to point to wrong locations. This resulted in 12 programs timing out (infinite loops) or segfaulting. Solution: Modified `emit_vector_get`, `emit_string_get`, and map get code to use `usize::MAX` as a sentinel for unpat ched jumps, then added post-liveness patching logic that scans for these sentinels and patches them to correct positions. **Result: All 46/46 test programs now pass** (was 34/46), all 163 unit tests pass, zero regressions.
  - **6.4.13 Spillover ergonomics:** Introduce destructuring helpers or higher-order utilities that lean on the new containers, and wire telemetry harnesses to capture allocator pressure under mixed workloads.
