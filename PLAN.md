...
- ✅ **6.4.1 Vector runtime primitives:** Designed a heap-backed vector layout with runtime helpers for create/access/clone/free and surfaced interpreter support for `vec`, `count`, `get`, and `subs`-style slicing, mirroring string semantics with borrowed inputs and cloned returns. Telemetry workloads (`churn_reuse`, `escaping_strings`, `mixed_sizes`) ran cleanly via `tests/programs/memory/run_allocator_telemetry.sh` with zero outstanding allocations.
  ...
  - ⚠️ **6.4.13 Multi-pass compilation for type inference (DISABLED):** Implemented three-pass compilation strategy to properly infer parameter types from call sites. **Pass 1:** Register all function signatures. **Pass 2:** Compile into temporary program gathering type information from call sites via `record_function_parameter_type` with propagation back to parent context. **Pass 3:** Recompile with known parameter types so `str()` and other operations use correct runtime functions based on actual types instead of defaults. Removed the previous hack that assumed `Any`-typed parameters were strings in `str()` context (src/compiler/mod.rs:862-872). Added type propagation from function scopes to parent context (src/compiler/functions.rs:103-110). **Status:** Successfully fixes original bug (debug_bug1.slisp) but introduces 5 new test regressions due to subtle codegen issue with nested let bindings. **Temporarily disabled** at src/compiler/mod.rs:151 pending bug investigation. Current: 42/46 passing. With multi-pass: 37/46 passing. See BUG_FIX_SUMMARY.md and MULTIPASS_BUG_INVESTIGATION.md for details.
